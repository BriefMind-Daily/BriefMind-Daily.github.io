...
let preloadedData = {
    ...
    selectedFields: new Set(['LLM']),  // 默认显示全部领域
...
function renderWechatArticles(articles) {
    ...
    articles.forEach((article, index) => {
        ...
        if (tagContainer) {
            ...
            copyButton.innerHTML = '<i class="fas fa-paper-plane"></i> 分享';
            copyButton.title = '分享文章信息';
            copyButton.addEventListener('click', function() {
                const title = article.report_title || article.title || '无标题';
                const url = article.url || '';
                const summary = article.summary || '';
                copyCardContent(title, url, summary);
            });
            tagContainer.appendChild(copyButton);
        }
...
function renderCombinedPapers(filteredPapers, filteredArxivPapers) {
    ...
    papersToRender.forEach(paper => {
        ...
        if (title) {
            ...
            if (normalizedTitle) {
                uniquePapersMap.set('title:' + normalizedTitle, paper);
            } else {
                // 如果标准化后标题为空，回退到使用PDF链接
                const pdfLink = paper['PDF链接'] || '';
                if (pdfLink) {
                    uniquePapersMap.set('pdf:' + pdfLink, paper);
                }
            }
        } else {
            // 如果没有标题，使用PDF链接作为键
    ...
    // 然后添加Arxiv论文，如果标准化标题已存在则跳过
    arxivPapersToRender.forEach(paper => {
        const title = paper['标题'] || '';
        if (title) {
            const normalizedTitle = normalizeTitle(title);
            if (normalizedTitle && !uniquePapersMap.has('title:' + normalizedTitle)) {
                uniquePapersMap.set('title:' + normalizedTitle, paper);
            } else if (!normalizedTitle) {
                // 如果标准化后标题为空，回退到使用PDF链接
                const pdfLink = paper['PDF链接'] || '';
                if (pdfLink && !uniquePapersMap.has('pdf:' + pdfLink)) {
                    uniquePapersMap.set('pdf:' + pdfLink, paper);
                }
            }
        } else {
    ...
    combinedPapers.forEach(paper => {
        ...
        if (tagContainer) {
            ...
            copyButton.className = 'copy-content-btn';
            copyButton.innerHTML = '<i class="fas fa-paper-plane"></i> 分享';
            copyButton.title = '分享论文信息';
            copyButton.addEventListener('click', function() {
                const title = paper['标题'] || '无标题';
                const url = paper['PDF链接'] || '';
                const summary = paper['简明摘要'] || '';
                copyCardContent(title, url, summary);
            });
            tagContainer.appendChild(copyButton);
        }
...
function getCachedData() {
    try {
        ...
        const cachedData = {
            ...
            fields: new Set(),
            institutions: new Set(),
            selectedFields: new Set(['all']),  // 默认选中LLM领域
            selectedInstitutions: new Set(['all'])
        };
...
function resetFilters() {
    ...
    preloadedData.selectedFields.add('all');  // 默认显示全部领域
...
// 新增函数：复制卡片内容
function copyCardContent(title, url, summary) {
    // 创建一个包含格式化内容的字符串
